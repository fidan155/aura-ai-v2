  ...
  return (
    <div className="bg-[#0A0D12] text-[#ECEFF3] min-h-screen selection:bg-[#F5A623] selection:text-[#0A0D12] overflow-x-hidden relative">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#0A0D12]/80 backdrop-blur-xl border-b border-[#1E2530] px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-mono text-[10px] text-[#7C8494]">v2.6 // ADMIN</span>
          </div>

          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-red-500/20 bg-red-500/10">
              <ShieldAlert className="w-3.5 h-3.5 text-red-400" />
              <span className="font-mono text-[11px] text-red-400 uppercase tracking-wide">Sicherer Knoten aktiv</span>
            </div>
            <button
      ...
      <main className="max-w-7xl mx-auto pt-32 pb-24 px-6 relative z-10 space-y-12">
        <motion.div {...fadeIn}>
          <h1 className="font-display font-bold text-4xl tracking-tight mb-4">Admin-Kontrollzentrum</h1>
          <p className="text-[#7C8494] max-w-xl">Überwache die KI-Nutzeranalysen und verwalte eingehende Systemanträge.</p>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-semibold text-lg">
              KI-Nutzer-Radar <span className="font-mono text-xs font-normal text-[#7C8494]">(Nutzung & Abwanderungsrisiko)</span>
            </h3>
          </div>

          {loadingKi ? (
            <div className="text-center py-8 font-mono text-xs text-[#7C8494]">Lade KI-Verhaltensdaten...</div>
          ) : userAnalysen.length === 0 ? (
            <div className="text-center py-8 font-mono text-xs text-[#7C8494]">Keine Nutzer-Verbindungen getrackt.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {userAnalysen.map((u) => {
                const badgeColor =
                  u.kiStatus === 'Active' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                  u.kiStatus === 'At Risk' ? 'bg-[#F5A623]/10 border-[#F5A623]/20 text-[#F5A623]' :
                ...
                return (
                  <div key={u.id} className="p-4 rounded-xl border border-[#1E2530] bg-[#0A0D12] flex flex-col justify-between space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-sm font-semibold text-[#ECEFF3] truncate max-w-[200px]">{u.email}</p>
                    ...
                    <div className="space-y-1.5 text-xs">
                      <p className="text-[#7C8494] leading-relaxed">
                        <strong className="text-[#ECEFF3]">Analyse:</strong> {u.kiSummary}
                      </p>
                      <p className="text-[#4CC9F0] font-mono text-[11px]">
                        <strong className="text-[#4CC9F0]">💡 Aktion:</strong> {u.kiRecommendation}
                      </p>
                    </div>
                  </div>
                );
        ...
        >
          <h3 className="font-display font-semibold text-lg mb-4">Eingehende Anträge der Nutzer</h3>
            ...
            <Table className="">
              <TableHeader className="border-[#1E2530] bg-[#12161D]">
                <TableRow className="border-[#1E2530] hover:bg-transparent">
                  ...
                  <TableHead className="text-center font-mono text-[#4CC9F0]">Status</TableHead>
                  <TableHead className="text-right font-mono text-[#7C8494]">Aktion</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody className="">
                {antraegeListe.length === 0 ? (
                  <TableRow className="border-[#1E2530]">
                    <TableCell colSpan={5} className="text-center text-[#7C8494] py-8 font-mono text-sm">
                  ...
                  antraegeListe.map((antrag) => {
                    ...
                    const statusColor =
                      normalisierterStatus === 'genehmigt' ? 'text-[#4CC9F0]' :
                    ...
                    return (
                      <TableRow key={antrag.id} className="border-[#1E2530] hover:bg-white/[0.02]">
                        ...
                        <TableCell className={`text-center font-semibold font-mono ${statusColor}`}>
                          {statusText}
                        </TableCell>
                        <TableCell className="text-right space-x-2">
                          <button
                            onClick={() => handleStatusUpdate(antrag.id, 'genehmigt')}
                          ...
                          <button
                            onClick={() => handleStatusUpdate(antrag.id, 'abgelehnt')}
